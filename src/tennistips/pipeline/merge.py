...
def load_tennisdata_raw(raw_dir: str) -> pd.DataFrame:
    ...
    for f in files:
        df = pd.read_csv(os.path.join(raw_dir, f))
        # tennis-data often uses Winner/Loser columns and tournament/date like Date, Tournament, Surface
        rename_map = {
            "Date":"date","Surface":"surface","Tournament":"tournament","Winner":"winner","Loser":"loser","Round":"round"
        }
        for k in rename_map:
            if k not in df.columns:
                # skip non-standard files
                return pd.DataFrame(columns=["date","surface","winner","loser","round","tournament"])
        df = df.rename(columns=rename_map)
        df["date"] = pd.to_datetime(df["date"], dayfirst=True, errors="coerce")
        # odds columns vary; we try common bookies
        odds_cols = [c for c in df.columns if c.lower().startswith("b") or "Avg" in c or "PS" in c]
        # Create simple two-way mean odds if available
...
def fuzzy_merge_results_and_odds(results: pd.DataFrame, odds: pd.DataFrame, threshold: int = 92) -> pd.DataFrame:
    ...
    for i, row in res.iterrows():
        ...
        if best_score >= threshold:
            ...
            # Assign odds consistently: player1 is the winner in res
...
def build_merged_dataset(cfg_path: str) -> str:
    ...
    if merged.empty:
        # fallback: salvar apenas resultados sem odds (modelo ainda dá para treinar)
        merged = res.rename(columns={"winner":"player1","loser":"player2"})
        merged["odds_p1"] = np.nan; merged["odds_p2"] = np.nan
